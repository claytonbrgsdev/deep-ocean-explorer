...
import { ocean, WORLD } from "@/lib/ocean"
...
const DRAG = 1.6
const MAX_SPEED = 7
const IDLE_PULSE_HZ = 1.1
const SWIM_PULSE_HZ = 2.6
...
export default function PlayerJellyfish() {
  ...
  useFrame((state, dt) => {
    ...
    // thrust only during the contraction half of the cycle
    const contraction = Math.max(0, Math.sin(phase))
    const thrust = contraction * contraction

    if (hasInput) {
      ocean.playerVel.addScaledVector(s.dir, ACCEL * thrust * delta)
    }
    // idle buoyancy bob
    ...
    // --- orientation: bell apex leads the motion ---
    if (speed > 0.35) {
    ...
    // --- feed the body shaders (3 uniform writes, zero allocations) ---
    body.uniforms.time.value = t
    body.uniforms.phase.value = phase
    body.uniforms.amp.value = 0.1 + 0.1 * Math.min(1, speed / 3)
    ...
    body.uniforms.glowBoost.value = 0.25 + 0.35 * Math.min(1, speed / MAX_SPEED)

    if (lightRef.current) {
      lightRef.current.intensity = 2.2 + contraction * 2.4
    }
